...
// 	global $post;
// 	$pattern ="/<img(.*?)class=\"(.*?)\"(.*?)>/i";
// 	$replacement = '<img$1class="$2 h-auto mv3 center db"$3>';
...
/**
 * Remove all height and width attrs
 */
...
/**
 * Get raw text without tags for except for posts
 */
add_action( 'rest_api_init', function () {
	register_rest_field(
		...
		'excerpt',
		array(
			'get_callback'    => 'get_raw_except_post',
...
function get_raw_except_post ( $object, $field_name, $request ) {
	global $post;
	$post = get_post ($object['id']);
	// This is what we currently have...
	$output['rendered'] = apply_filters( 'the_content',  get_the_excerpt());
	// This includes the shortcodes/tags/etc
	$output['_raw'] = get_the_excerpt();
	return $output;
}

/**
 * Get raw text without tags for except for pages
 */
add_action( 'rest_api_init', function () {
	register_rest_field(
		'page',
		'excerpt',
		array(
			'get_callback'    => 'get_raw_except_page',
			'update_callback' => null,
			'schema'          => null,
		)
	);
...
function get_raw_except_page ( $object, $field_name, $request ) {
	global $page;
	$page = get_page ($object['id']);
	// This is what we currently have...
	$output['rendered'] = apply_filters( 'the_content',  get_the_excerpt());
	// This includes the shortcodes/tags/etc
	$output['_raw'] = get_the_excerpt();
